...
async function applyLiveOverlay(){
  try{
    const res = await Promise.all(["goodra","dragonite"].map(q =>
      fetch(`https://api.pokemontcg.io/v2/cards?q=name:*${q}*&pageSize=250&select=id,tcgplayer,cardmarket`,
        {signal: AbortSignal.timeout(9000)}).then(r=>r.json())));
    const ix = {};
    res.flatMap(r=>r.data||[]).forEach(c=>ix[c.id]=c);
